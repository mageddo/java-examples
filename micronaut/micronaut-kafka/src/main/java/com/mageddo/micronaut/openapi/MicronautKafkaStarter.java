...
public class MicronautKafkaStarter {
  ...
  public void ping() {
    kafkaProducer.send(
        new ProducerRecord<>("ping", LocalDateTime.now()
        .toString()
        .getBytes())
    );
  ...
  public void news() {
    kafkaProducer.send(
        new ProducerRecord<>("news", LocalDateTime.now()
        .toString()
        .getBytes())
    );
  ...
  public void coffeeRequester() {
    kafkaProducer.send(new ProducerRecord<>("coffee_request", LocalDateTime.now()
        .toString()
        .getBytes()));
